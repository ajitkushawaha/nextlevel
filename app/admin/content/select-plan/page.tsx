...
import { Save, RefreshCw, Plus, Trash2, ArrowLeft } from 'lucide-react'
import Link from 'next/link'
import { toast } from 'sonner'

interface SelectPlanContent {
  _id?: string
  trustSection: {
    mainText: string
    features: Array<{
...
export default function SelectPlanContentManagement() {
  const [content, setContent] = useState<SelectPlanContent | null>(null)
  ...
  const fetchContent = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/admin/select-plan-content')
      const data = await response.json()

      if (data.success) {
        setContent(data.selectPlanPage)
      } else {
        toast.error(data.error || 'Failed to fetch select-plan content')
      }
    } catch (error) {
      console.error('Error fetching select-plan content:', error)
      toast.error('Failed to fetch select-plan content')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleFeatureChange = (index: number, value: string) => {
    ...
    const updatedFeatures = [...content.trustSection.features]
  ...
  const addFeature = () => {
    ...
    setContent(prev => ({
      ...
      trustSection: {
        ...prev!.trustSection,
        features: [
          ...prev!.trustSection.features,
          {
            text: '',
            order: prev!.trustSection.features.length,
            status: 'active',
          },
        ],
      },
  ...
  const removeFeature = (index: number) => {
    ...
    const updatedFeatures = content.trustSection.features.filter(
      (_, i) => i !== index
    )
  ...
  const toggleFeatureStatus = (index: number) => {
    if (!content) return
    ...
    updatedFeatures[index] = {
      ...updatedFeatures[index],
      status:
        updatedFeatures[index].status === 'active' ? 'inactive' : 'active',
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success('Select-plan content updated successfully!')
        await fetchContent()
      } else {
        const error = await response.json()
        toast.error(error.error || 'Failed to save select-plan content')
      }
    } catch (error) {
      console.error('Error saving select-plan content:', error)
      toast.error('Failed to save select-plan content')
    } finally {
      setSaving(false)
    }
  }
      ...
      <div className="flex-1 bg-slate-200">
        <div className="p-8">
          <div className="text-center">
            ...
            <p className="text-gray-600">Unable to load select-plan content.</p>
          </div>
        </div>
      </div>
    )
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">
                Select Plan Page Content
              </h1>
              <p className="text-gray-600 mt-2">
                Manage the trust section content displayed on the select-plan
                page
              </p>
            </div>
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Manage the main text and features displayed in the trust section
                at the bottom of the select-plan page
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="mainText"
                  value={content.trustSection.mainText}
                  onChange={e => handleMainTextChange(e.target.value)}
                  placeholder="Visa4 has brought joy to over 1,50,000 happy travellers!"
                />
                <p className="text-sm text-gray-500">
                  This text appears at the top of the trust section with an icon
                </p>
              </div>
              ...
              <div className="space-y-4">
                ...
                {content.trustSection.features.map((feature, index) => (
                  <div
                    key={index}
                  ...
                  >
                    <div className="flex-1">
                      <Input
                        ...
                        placeholder="e.g., 100% Secure Processing"
                      />
                    </div>
                    <Badge
                      variant={
                ...
                {content.trustSection.features.length === 0 && (
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                How the trust section will appear on the select-plan page
              </CardDescription>
            </CardHeader>
            <CardContent>
